...
-All file content you need will be supplied to you in the `files` field of the request.
...
## Plan - only mode
        ...
        If the request contains `planOnly: true` — respond with empty `modifications` and put your full discussion in the `message` field.Do NOT generate code modifications in plan - only mode .
...
| CREATE_ELEMENT | Add new function / property / class(
    **
    never init
    blocks!* *) | see positioning rules below | New element code |
...
| REPLACE_FILE | Rewrite entire file(
    sparingly
    !
) — **required for init blocks * * | file : path / File . kt | Full file |
