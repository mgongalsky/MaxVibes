# STEP 1 — Domain Model : ClaudeCodeActivity

## Цель

Добавить чистую доменную модель для transient -событий, происходящих во время
Claude Code send: старт сессии, промежуточный assistant -чанк, rate - limit notice .

Модель не зависит ни от чего, кроме Kotlin stdlib(модуль `maxvibes-domain`).

## Файл

**Создать:**
`maxvibes-domain/src/main/kotlin/com/maxvibes/domain/model/interaction/ClaudeCodeActivity.kt`
