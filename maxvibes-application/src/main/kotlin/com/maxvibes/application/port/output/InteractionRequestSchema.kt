...
object InteractionRequestSchema {
    ...
    /**
     * Поле, в которое модель выносит адрес элемента отдельно от файла. Каноном не является:
     * адрес принадлежит [MOD_PATH] целиком. Принимается только чтобы склеить его с путём
     * файла — иначе операция над элементом приходит с адресом файла и означает совсем не то,
     * что модель имела в виду.
     */
    val MOD_ELEMENT_PATH = "elementPath"

    /**
     * Имена, под которыми модель может прислать полное имя импорта; каноническое — первое.
     * `import` — не догадка: так это поле называется в самом Kotlin, и модель пишет его
     * по аналогии с кодом. Без синонима ADD_IMPORT доезжал с пустым FQN и молча
     * отбрасывался конвертером — правка исчезала, а проверки шли по неизменённому файлу.
     */
    val MOD_IMPORT_PATH_KEYS = listOf(MOD_IMPORT_PATH, "import", "importFqName")
