 ...
 * Anonymous object literals terminate the climb.
 *
 * Inverse operation of [PsiNavigator]. Must be called inside a read action. Stateless.
 */
object ElementAtCaretResolver {

    data class CaretElement(
        /** Full path like file:src/.../Foo.kt/class[Foo]/function[bar]; null when nothing addressable. */
        val elementPath: String?,
        val elementName: String?,
        /** Leaf kind label: function / property / class / interface / object / companion_object / enum_entry. */
        val kind: String?,
        /** Full source text of the resolved declaration (used for attach-element). */
    ...
    fun resolve(psiFile: PsiFile, offset: Int, projectRelativePath: String): CaretElement {
        ...
        return CaretElement(
            ...
            // Unnamed companion objects have a null PSI name — fall back to the kind label
            // so the action layer never rejects a perfectly addressable companion.
            elementName = target.name ?: kindLabel(target),
